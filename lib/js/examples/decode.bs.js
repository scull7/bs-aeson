...
function mapJsonObjectString(f, decoder, encoder, str) {
  ...
  return JSON.stringify(Js_dict.map(Curry.__1(encoder), Js_dict.map(Curry.__1(f), Aeson_decode$BsAeson.dict(decoder, json))));
}
...
try {
  x = Aeson_decode$BsAeson.field("x", Aeson_decode$BsAeson.$$int, json);
  exit = 1;
}
catch (raw_exn){
  var exn = Js_exn.internalToOCamlException(raw_exn);
  if (exn[0] === Aeson_decode$BsAeson.DecodeError) {
